...
if (class_exists('WP_Customize_control')) {
    ...
    class Education_Method_Repeater_Controler extends WP_Customize_Control
    {
        /**
         * The control type.
         *
        ...
        public $education_method_box_label = '';

        public $education_method_box_add_control = '';
        ...
        {
            $this->fields = $fields;
            $this->education_method_box_label = $args['education_method_box_label'];
            $this->education_method_box_add_control = $args['education_method_box_add_control'];
        ...
        <?php } ?>
            ...
            <ul class="education_method-repeater-field-control-wrap">
                <?php
                $this->education_method_get_fields();
                ?>
            </ul>

            <input type="hidden" <?php $this->link(); ?> class="education_method-repeater-collector"
                   value="<?php echo esc_attr($this->value()); ?>"/>
            <button type="button"
                    class="button education_method-add-control-field"><?php echo esc_html($this->education_method_box_add_control); ?></button>
            <?php
        }

        private function education_method_get_fields()
        {
            $fields = $this->fields;
            $values = json_decode($this->value());
            ...
            if (is_array($values)) {
                foreach ($values as $value) {
                    ?>
                    <li class="education_method-repeater-field-control">
                        <h3 class="education_method-repeater-field-title"><?php echo esc_html($this->education_method_box_label); ?></h3>

                        <div class="education_method-repeater-fields">
                            <?php
                            foreach ($fields as $key => $field) {
                                $class = isset($field['class']) ? $field['class'] : '';
                                ?>
                                <div
                                        class="education_method-fields education_method-type-<?php echo esc_attr($field['type']) . ' ' . $class ?>">
                                    ...
                                    switch ($field['type']) {
                                        ...
                                        case 'upload':
                                            ...
                                            echo '<div class="education_method-fields-wrap">';
                                            echo '<div class="attachment-media-view">';
                                            echo '<div class="placeholder' . $image_class . '">';
                                            esc_html__('No image selected', 'education-method');
                                            echo '</div>';
                                            echo '<div class="thumbnail thumbnail-image">';
                                            echo $image;
                                            echo '</div>';
                                            echo '<div class="actions clearfix">';
                                            echo '<button type="button" class="button education_method-delete-button align-left">' . esc_html__('Remove', 'education-method') . '</button>';
                                            echo '<button type="button" class="button education_method-upload-button alignright">' . esc_html__('Select Image', 'education-method') . '</button>';
                                        ...
                                        case 'category':
                                            ...
                                            echo '<option value="0">' . esc_html__('Select Category', 'education-method') . '</option>';
                                            echo '<option value="-1">' . esc_html__('Latest Posts', 'education-method') . '</option>';
                                        ...
                                        case 'colorpicker':
                                            echo '<input data-default="' . esc_attr($default) . '" class="education_method-color-picker" data-alpha="true" data-name="' . esc_attr($key) . '" type="text" value="' . esc_attr($new_value) . '"/>';
                                            break;
                                        ...
                                        case 'range':
                                            ...
                                            echo '<div class="education_method-range-slider" >';
                                        ...
                                        case 'icon':
                                            echo '<div class="education_method-selected-icon">';
                                            echo '<i class="' . esc_attr($new_value) . '"></i>';
                                            echo '<span><i class="fa fa-angle-down"></i></span>';
                                            echo '</div>';
                                            echo '<ul class="education_method-icon-list clearfix">';
                                            $education_method_font_awesome_icon_array = education_method_font_awesome_icon_array();
                                            foreach ($education_method_font_awesome_icon_array as $education_method_font_awesome_icon) {
                                                $icon_class = $new_value == $education_method_font_awesome_icon ? 'icon-active' : '';
                                                echo '<li class=' . $icon_class . '><i class="' . $education_method_font_awesome_icon . '"></i></li>';
                                            }
                                            echo '</ul>';
                                            echo '<input data-default="' . esc_attr($default) . '" type="hidden" value="' . esc_attr($new_value) . '" data-name="' . esc_attr($key) . '"/>';
                                            break;
                                        ...
                                        case 'multicategory':
                                            ...
                                            echo '<ul class="education_method-multi-category-list">';
                                            echo '<li><label><input type="checkbox" value="-1" ' . checked('-1', $new_value, false) . '/>' . esc_html_e('Latest Posts', 'education-method') . '</label></li>';
                                            foreach ($this->cats as $cat) {
                            ...
                            <div class="clearfix education_method-repeater-footer">
                                <div class="alignright">
                                    <a class="education_method-repeater-field-remove"
                                       href="#remove"><?php esc_html_e('Delete', 'education-method') ?></a> |
                                    <a class="education_method-repeater-field-close"
                                       href="#close"><?php esc_html_e('Close', 'education-method') ?></a>
                                </div>
                            </div>
                        </div>
                    </li>
...
if (!function_exists('education_method_font_awesome_icon_array')) {
    function education_method_font_awesome_icon_array()
    {
